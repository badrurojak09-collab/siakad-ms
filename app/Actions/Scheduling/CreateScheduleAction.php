    ...
    public function execute(array $data): Schedule
    {
        ...
        if (!empty($data['is_online'])) {
            ...
            $conflict = Schedule::query()
                ...
                ->exists();
            if ($conflict)
                throw ValidationException::withMessages(['room_id' => 'Ruang sudah dipakai pada waktu yang beririsan.']);
        }
        ...
        if (!empty($data['lecturer_id'])) {
            ...
            if ($conflict)
                throw ValidationException::withMessages(['lecturer_id' => 'Dosen sudah memiliki jadwal yang beririsan.']);
        }
