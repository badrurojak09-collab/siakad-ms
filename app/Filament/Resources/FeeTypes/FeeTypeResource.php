...
class FeeTypeResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            Section::make('Informasi Jenis Biaya')
                ->description('Data Jenis Biaya Tagihan Mahasiswa')
                ->schema([
                    TextInput::make('code')->label('Kode')->required()->maxLength(50)->unique(ignoreRecord: true),
                    TextInput::make('name')->label('Nama Biaya')->required()->maxLength(150),
                    TextInput::make('default_amount')->label('Nominal Default')->numeric()->minValue(0)->required(),
                    Select::make('frequency')->label('Frekuensi')->options(['one_time' => 'Sekali Bayar', 'per_semester' => 'Per Semester', 'per_year' => 'Per Tahun'])->default('per_semester')->required(),
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            TextColumn::make('code')
                ->label('Kode')->searchable(),
            TextColumn::make('name')->label('Nama'),
            TextColumn::make('default_amount')->label('Nominal')->money('IDR'),
            TextColumn::make('frequency')->label('Frekuensi')->badge(),
            TextColumn::make('is_active')->label('Status')->formatStateUsing(fn($s) => $s ? 'Aktif' : 'Tidak Aktif')->badge()
        ])
            ->actions([
