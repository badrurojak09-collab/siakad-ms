...
class AdmissionBillResource extends Resource
{
    ...
    {
        return $table->columns([TextColumn::make('bill_number')->label('Nomor'), TextColumn::make('applicant.registration_number')->label('Pendaftar'), TextColumn::make('amount')->label('Nominal')->money('IDR'), TextColumn::make('paid_amount')->label('Terbayar')->money('IDR'), TextColumn::make('status')->label('Status')->badge(), TextColumn::make('due_date')->label('Jatuh Tempo')->date()])->actions([EditAction::make()->label('Ubah')->visible(fn(AdmissionBill $r) => $r->status !== 'paid'), DeleteAction::make()->label('Hapus')->visible(fn(AdmissionBill $r) => $r->paid_amount == 0)->requiresConfirmation()]);
    }

    public static function getPages(): array
    {
        return ['index' => Pages\ListAdmissionBills::route('/'), 'create' => Pages\CreateAdmissionBill::route('/create'), 'edit' => Pages\EditAdmissionBill::route('/{record}/edit')];
    }
}
