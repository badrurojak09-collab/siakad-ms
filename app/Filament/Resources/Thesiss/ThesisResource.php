...
class ThesisResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            ...
            DateTimePicker::make('defense_date')->disabled(), TextInput::make('defense_room')->maxLength(100)->disabled(), TextInput::make('final_document_url')->url()->maxLength(500),
        ]);
    }
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            TextColumn::make('student.nim')->label('NIM')->searchable(), TextColumn::make('title')->searchable()->wrap(), TextColumn::make('supervisor1.nidn')->label('Pembimbing 1'),
            TextColumn::make('status')->badge()->sortable(), TextColumn::make('defense_date')->dateTime(), TextColumn::make('created_at')->dateTime()->sortable(),
        ])->actions([
            \Filament\Tables\Actions\EditAction::make()->visible(fn (Thesis $record): bool => !in_array($record->status, ['completed', 'cancelled'], true)),
            \Filament\Tables\Actions\DeleteAction::make()->visible(fn (Thesis $record): bool => $record->status === 'proposed'),
        ])->defaultSort('created_at', 'desc');
    }

    public static function getRelations(): array { return [SupervisionsRelationManager::class]; }
