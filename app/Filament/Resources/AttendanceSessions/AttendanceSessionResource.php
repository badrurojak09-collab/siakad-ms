...
class AttendanceSessionResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            ...
            DatePicker::make('meeting_date')->label('Tanggal Pertemuan')->required()->native(false),
            TextInput::make('meeting_number')->label('Pertemuan Ke-')->numeric()->minValue(1)->maxValue(32)->required(),
            TextInput::make('topic')->label('Topik')->maxLength(255)
        ]);
    }
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            ...
            TextColumn::make('meeting_date')->label('Tanggal')->date('d M Y'),
            TextColumn::make('meeting_number')->label('Pertemuan Ke-')->sortable(),
            TextColumn::make('topic')->label('Topik')->searchable(),
            TextColumn::make('opened_at')->label('Dibuka')->dateTime('d M Y H:i'),
            TextColumn::make('closed_at')->label('Ditutup')->dateTime('d M Y H:i')
        ])->actions([EditAction::make()->label('Ubah')->visible(fn(AttendanceSession $r) => $r->closed_at === null), DeleteAction::make()->label('Hapus')->visible(fn(AttendanceSession $r) => $r->opened_at === null)->requiresConfirmation()]);
    }
