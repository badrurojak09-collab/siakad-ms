...
class TeachingAssignmentResource extends Resource
{
    ...
    {
        return $table->columns([TextColumn::make('courseClass.class_code')->label('Kelas'), TextColumn::make('lecturer.user.name')->label('Dosen'), TextColumn::make('role')->label('Peran')->badge(), TextColumn::make('teaching_load')->label('Beban'), TextColumn::make('status')->label('Status')->badge()])->actions([EditAction::make()->label('Ubah'), DeleteAction::make()->label('Hapus')->requiresConfirmation()]);
    }

    public static function getPages(): array
    {
        return ['index' => Pages\ListTeachingAssignments::route('/'), 'create' => Pages\CreateTeachingAssignment::route('/create'), 'edit' => Pages\EditTeachingAssignment::route('/{record}/edit')];
    }
}
