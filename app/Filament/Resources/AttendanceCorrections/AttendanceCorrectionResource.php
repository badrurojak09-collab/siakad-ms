...
class AttendanceCorrectionResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            Select::make('attendance_record_id')->label('Rekaman Presensi')->relationship('attendanceRecord', 'id')->searchable()->preload()->required(),
            Select::make('requested_by')->label('Diajukan Oleh')->relationship('requestedBy', 'name')->searchable()->preload()->required(),
            Textarea::make('reason')->label('Alasan Koreksi')->required()->columnSpanFull(),
            Select::make('status')->label('Status')->options(['pending' => 'Menunggu', 'approved' => 'Disetujui', 'rejected' => 'Ditolak'])->default('pending')->required(),
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            TextColumn::make('attendanceRecord.student.nim')->label('NIM'),
            TextColumn::make('requestedBy.name')->label('Pengaju'),
            TextColumn::make('reason')->label('Alasan')->limit(50),
            TextColumn::make('status')->label('Status')->badge(),
            TextColumn::make('approvedBy.name')->label('Penyetuju')->placeholder('-')
