...
class MbkmActivityResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            ...
            TextColumn::make('credits_recognized')->label('SKS'),
            TextColumn::make('status')->badge(),
        ])->actions([
            EditAction::make()->visible(fn(MbkmActivity $record): bool => in_array($record->status, ['pending', 'rejected'], true)),
            DeleteAction::make()->visible(fn(MbkmActivity $record): bool => $record->status === 'pending'),
        ])->defaultSort('created_at', 'desc');
    }

    public static function getPages(): array
    {
        return ['index' => Pages\ListMbkmActivitys::route('/'), 'create' => Pages\CreateMbkmActivity::route('/create'), 'edit' => Pages\EditMbkmActivity::route('/{record}/edit')];
    }
}
