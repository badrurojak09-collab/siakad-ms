...
class GraduationResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            ...
            Action::make('mark_eligible')->label('Tandai Memenuhi Syarat')->icon(Heroicon::OutlinedCheckCircle)->requiresConfirmation()->visible(fn (Graduation $record): bool => $record->status === 'proposed')->action(function (Graduation $record): void { $record->update(['status' => 'eligible']); Notification::make()->title('Status kelulusan diperbarui')->success()->send(); }),
    ...
    public static function getPages(): array { return ['index'=>Pages\ListGraduations::route('/'),'create'=>Pages\CreateGraduation::route('/create'),'edit'=>Pages\EditGraduation::route('/{record}/edit')]; }
}
